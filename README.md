...
**SAG**是SQL驱动的新一代 RAG 引擎，可以在运行时自动构建知识图谱
...
- **自然语言向量（Natural Language Vector）**  
  *不只把整段文本编码成向量，而是为每个 Event 抽取多维属性：时间、地点、人物、动作、话题、标签…  
  它们组成了一个“**由自然语言属性构成的向量**”。*

**关键洞察**：  
- Event 是 **原子知识单元**  
- Entity 是 **事件的属性维度**  
- 实体之间的关系不提前计算，而是 **在查询时动态计算**
...
- **步骤概要**：
  - **LLM 解析查询**：抽取结构化属性（TOPIC、ACTION、PERSON…）
  - **向量检索实体**：在实体向量空间中搜索  
