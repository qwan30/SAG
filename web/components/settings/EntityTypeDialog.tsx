...
export function EntityTypeDialog({
  ...
  const validateForm = (): boolean => {
    ...
    if (!formData.type.trim()) {
      newErrors.type = '请输入类型标识符'
    } else if (!/^[a-z][a-z0-9_]*$/.test(formData.type)) {
      newErrors.type = '只能包含小写字母、数字和下划线，且必须以字母开头'
    }
    ...
    if (formData.value_constraints) {
      ...
      if (formData.value_constraints.type === 'enum') {
        if (!formData.value_constraints.enum_values || formData.value_constraints.enum_values.length === 0) {
          newErrors.value_constraints = '枚举类型至少需要一个可选值'
        }
      }
      ...
      if (['int', 'float'].includes(formData.value_constraints.type)) {
        ...
        if (min !== undefined && max !== undefined && min > max) {
          newErrors.value_constraints = '最小值不能大于最大值'
        }
      }
    }
          ...
          <form onSubmit={handleSubmit} className="overflow-y-auto scrollbar-thin">
            ...
            <div className="space-y-2.5">
              <Label htmlFor="type-input" className="text-sm font-medium text-gray-800">
                类型标识符 <span className="text-red-500">*</span>
              </Label>
              <Input
                id="type-input"
                type="text"
                ...
                disabled={isEditMode}
                placeholder="例如: company, project_stage"
              ...
              {isEditMode && (
                <p className="text-xs text-gray-500 mt-1.5">类型标识符创建后不可修改</p>
              )}
            </div>
            ...
            <div className="space-y-2.5">
              ...
              <Textarea
                ...
                placeholder="用于指导 AI 提取该类型实体的描述"
                rows={3}
            ...
            <Collapsible open={isValueTypeOpen} onOpenChange={setIsValueTypeOpen}>
              <CollapsibleTrigger className="flex items-center justify-between w-full p-3 bg-gray-50 hover:bg-gray-100 rounded-lg transition-colors">
                <div className="flex items-center gap-2">
                  <span className="text-sm font-medium text-gray-800">值类型配置</span>
              ...
              <CollapsibleContent className="space-y-4 pt-4">
                ...
                <div className="space-y-2">
                  <Label className="text-sm font-medium text-gray-800">快速模板</Label>
                  <div className="grid grid-cols-2 gap-2">
                    ...
                    >
                      <div className="font-medium text-gray-800">💰 价格</div>
                      <div className="text-xs text-gray-500">浮点数 + 单位</div>
                    </button>
                    <button
                      type="button"
                      onClick={() => {
                      ...
                      className="px-3 py-2 text-sm bg-white border-2 border-gray-200 rounded-lg hover:border-gray-400 hover:bg-gray-50 transition-all text-left"
                    >
                      <div className="font-medium text-gray-800">📋 项目阶段</div>
                      <div className="text-xs text-gray-500">枚举类型</div>
                    </button>
                    <button
                      type="button"
                      onClick={() => {
                      ...
                      className="px-3 py-2 text-sm bg-white border-2 border-gray-200 rounded-lg hover:border-gray-400 hover:bg-gray-50 transition-all text-left"
                    >
                      <div className="font-medium text-gray-800">📅 日期</div>
                      <div className="text-xs text-gray-500">日期时间类型</div>
                    </button>
                    <button
                      type="button"
                      onClick={() => {
                      ...
                      className="px-3 py-2 text-sm bg-white border-2 border-gray-200 rounded-lg hover:border-gray-400 hover:bg-gray-50 transition-all text-left"
                    >
                      <div className="font-medium text-gray-800">✅ 状态</div>
                      <div className="text-xs text-gray-500">枚举类型</div>
                    </button>
                  </div>
                </div>
                ...
                <div className="space-y-2.5">
                  ...
                  >
                    ...
                    >
                      <SelectValue placeholder="选择值类型（留空则自动解析）" />
                    </SelectTrigger>
                    <SelectContent className="bg-white border-gray-200 shadow-lg">
                      <SelectItem value="none">不指定（自动解析）</SelectItem>
                ...
                {formData.value_constraints?.type === 'enum' && (
                  <div className="space-y-2.5">
                    <Label className="text-sm font-medium text-gray-800">
                      枚举可选值 <span className="text-red-500">*</span>
                    </Label>
                    <div className="space-y-2">
                ...
                {formData.value_constraints?.type && (
                  <div className="space-y-3 p-4 border-2 border-gray-200 rounded-lg bg-gray-50">
                    <Label className="text-sm font-medium text-gray-800">默认值</Label>
                    ...
                    {formData.value_constraints.default && (
                      <div className="flex items-center justify-between pt-2 border-t border-gray-200">
                        <div className="flex-1 pr-4">
                          <Label className="text-sm font-medium text-gray-800">强制模式</Label>
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="space-y-2.5">
                <Label htmlFor="threshold-input" className="text-sm font-medium text-gray-800">
                  相似度阈值 <span className="text-red-500">*</span>
                </Label>
                <Input
                  id="threshold-input"
                  type="number"
